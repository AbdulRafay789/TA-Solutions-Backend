...
import { Currency, ConversionResponse, FreeCurrencyApiResponse } from '../types';
...
export class CurrencyService {
    ...
    try {
      ...
      for (const [code, data] of Object.entries(response.data.data)) {
        currencies.push({
          ...
          symbol: (data as any).symbol || code
        });
      }
  ...
  async getExchangeRates(baseCurrency: string, targetCurrencies: string[]): Promise<Record<string, number>> {
    try {
      const response = await axios.get<FreeCurrencyApiResponse>(`${this.baseUrl}/latest`, {
        params: {
          apikey: this.apiKey,
